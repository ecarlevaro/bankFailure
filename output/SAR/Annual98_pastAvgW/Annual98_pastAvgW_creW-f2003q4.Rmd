...
-   $\bm{y_t}$ is 1 if the bank fails between `r specs$minExitDateQ` and `r specs$failureHorizon`.
...
banks <- banksDB %>%
  drop_na %>%
  filter(startDateQ<= !!specs$X$from & exitDateQ >= !!specs$y$minExitDateQ) 
```
...
The following entities appear in the network but not in the banks dataframe: `netExcBanks`. Should be empty!
...
## A list with many y vectors for each failure horizon
#Ys <- map(specs$failureHorizons, function(fHorizon) {
#  # 1: survival, 0: failure during the sample period
#  y <- if_else(banks$EXIT_DATE_Q <= fHorizon, 0, 1)
#  
#  y
#})
