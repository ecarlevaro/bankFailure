...
# This function redefines the summary method for ProbitSpatial object
# The method in ProbitSpatial:: does not return an object but prints directly
summary_probit_spatial_GM <- function(object, covar=FALSE, ...) {
...
# From the output of summary_probit_spatial_GM() calls texreg function
extract_ProbitSpatialGM <- function(model, useCovar=FALSE) {

  co <- coefficients(model)
  names <- names(co)
  # Std errors
  theS <- summary_probit_spatial_GM(model, covar = useCovar) # see the function below. it ouputs a matrix with std errors
  # Std errors are not available when using the LR test
  if (useCovar == FALSE) {
    # I return pvalue when using LR test, instead of the LR value theS[, 'LR test']
    se <- theS[, 'Pr(>z)']  
  } else {
    se <- theS[, 'Std. Error']
  }
...
setMethod('extract', 
          ...
          definition = extract_ProbitSpatialGM)
