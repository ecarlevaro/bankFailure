---
title: "Probit SAR with 2 regressors 1997q4-2000q4 same N"
author: "Emi"
date: "5/05/2021"
output: 
  html_document:
...
-   Only 2 regressors: ActivoN, C8Est_w

-   Failure at time t

-   Same N over time (hence, only banks alive at the beginning 1997q4).

-   Sample is 1997q4 to 2000q4.

-   W is fixed at annual average 1998 with all links (no restriction).
...
samsSpecs <- tribble(~name, ~start,
                     's_pool_97q4_00q4_priv_atBeg', 1997.4) 
                     #'s1998q2_priv', 1998.2,
                     #'s1998q3_priv', 1998.3,
                     #'s1998q4_priv', 1998.4,
                     #'s1999q1_priv', 1999.1,
...
samsSpecs <- mutate(samsSpecs, 
                    'failureUnitl' = 2001.4,
                    'predictorsUntil' = 2001.4,
                    'excludeStateOwn' = TRUE,
                    'outPath' = 'code/standardSpatialProbit/pool/basic_test')
```

The benchmark case I select all banks alive in 1997 and use their covariate to predict failure until time `r samsSpecs$failureUnitl[1]` .
...
r1 <- pmap(samsSpecs, function(...) {
  ...
  sam <- db %>% 
          ...
          select(IDENT, FECHAdata, GRUPO_ID_UNI, !!varsList) %>%
          # Only complete observations
          filter_all(all_vars(!is.na(.))) %>%
          # Only observations at this time
          filter(FECHAdata == samSpec$start)
          # ONly private banks
  ...
  Y <- if_else(sam$EXIT_DATE_Y <= samSpec$failureUntil, 0, 1)
  ...
  saveRDS(p, file=str_c( 'probits_quarterly_all_banks_p_', samSpec$name, '.rds'))
