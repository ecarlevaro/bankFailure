...
specs <- list('savingFolder'='output/SAR/Annual99/', 
              'idSample'= 'A99_Wtill98q4_creW_b97q4_s03q4')
...
```{r load_network}
netIg <- readRDS(paste0(specs$savingFolder, specs$idSample, '_network.rds'))
network <- readRDS(paste0(specs$savingFolder, specs$idSample, '_network.rds')) %>%
  as_tbl_graph() %>%
  activate(nodes) %>%
  rename('IDent' = 'name') %>%
  rename('label' = 'NOMRED') %>%
  mutate(degree = degree(.),
         ...
         value = ActivoN, 
         title = str_c(label, ', $ ', (ActivoN/10000), ' mill.'))
