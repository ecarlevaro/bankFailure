...
dbEnts <- haven::read_dta('C:/Users/emi.ABLE-22868/OneDrive/UWA PhD/bankFailure/data/failures/failure_time.dta') %>%
        dplyr::mutate(., 
          EXIT_DATE_Q = if_else(is.na(EXIT_DATE), 
                        9999, 
                        quarter(as_date(EXIT_DATE, origin=ymd('1960-01-01')), with_year=TRUE) ),
...
ggplot(data=failuresByQ, mapping=aes(x=Q, y=N_FAILS)) +
  geom_col() +
  scale_x_continuous(name='Quarters', breaks=failsByTime$Q) +
