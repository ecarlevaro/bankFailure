...
dbFcialEnts <- read_dta('./data/DBbanks/capital-allEntities-Monthly.dta', col_select = c(IDent, FECHAdata, bNombre, grupoIDUni, Activo, ActivoR,
                ...
                filter(., FECHAdata<ymd("2002-01-01") & FECHAdata>ymd("1997-05-01"))
```
...
```
The **318 Banco de Salta SA** which was absorbed by Banco Macro on November 1999. I have detailed data only till Sep-1999. I could get more aggregated data if necesary from the balance resumido.
**Caja de crédito Universo Cooperativa Limitada 64123** did not report its balance in 2000-10. I treat it as a missing value. 

I impute this missing values. 
...
Verify that all nodes involved in relations are in dbFcialEnts. 
The following table shows the relations in which a debtor does not appear in dbFcialEnts.
*anti_join()*, returns rows in *relations* without a match in *dbFcialEnts*.
```{r}
...
missDebtors
```
This could be due to debtors disappearing. A debtor disappears as a financial entitites but the debts do not expire, such that active financial entities still report the debts. 
...
atDate <- ymd("1999-12-01")
thisRelations <- dplyr::filter(relations, FECHAdata == atDate) %$% 
                  .[ !(.$IDENT_DEUDORA %in% missDebtors$IDENT_DEUDORA),  ] %>%
                  dplyr::mutate(., WEIGHT = PRESTAMOS/APRamplio) %>%
                  dplyr::filter(., WEIGHT>0.005) %>%
                  dplyr::select(., IDENT_ACREEDORA, IDENT_DEUDORA, WEIGHT)
...
```{r weight_density}
summarise(thisRelations[,'WEIGHT'],
          'Mean' = mean(WEIGHT),
          'Median' = median(WEIGHT),
          'SD' = sd(WEIGHT),
          '1st quartile' = quantile(WEIGHT, probs=.25))
...
```{r }
g <- graph_from_data_frame(thisRelations, directed = TRUE,
                           vertices = nodes)
```
