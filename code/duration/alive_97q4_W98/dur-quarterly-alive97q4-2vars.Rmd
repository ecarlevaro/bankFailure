---
title: "Duration probit SAR quarterly -2vars"
author: "Emi"
date: "14/05/2021"
output: 
  html_document:
...
```{r thisSample}
samsSpecs <- tribble(~name, ~start,
                     'dur_q_alive97q4_priv', 1997.4)
samsSpecs <- mutate(samsSpecs, 
                    'failureUnitl' = 2001.4,
                    'predictorsUntil' = 2000.4,
                    'excludeStateOwn' = TRUE,
                    'outPath' = 'code/duration/alive_97q4_W98')
```

The benchmark case I select all banks alive in 1997 and use their covariate to predict failure until time `r samsSpecs$end[1]` .

Here, I include banks alive on each t (instead of only banks alive on 1997q4).

Are state-owned banks excluded? `r samsSpecs$excludeStateOwn[1]`
...
```{r estimation}
r1 <- pmap(samsSpecs, function(...) {
  # samSpec <- samsSpecs[1,]
  samSpec <- tibble(...)
  # Bank covariates (X)
  varsList <- c('ActivoN', 'C8Est_w')
  sam <- db %>% 
          # Select observations of banks alive in 1997.4          
          filter(START_Q <= 1997.4 & EXIT_DATE_Q > 1997.4) %>%
          select(IDENT, FECHAdata, GRUPO_ID_UNI, EXIT_Q, START_Q, !!varsList) %>%
          # Only complete observations
          filter_all(all_vars(!is.na(.))) %>%
          # Only observations at this time
          filter(FECHAdata >= samSpec$start & FECHAdata <= samSpec$predictorsUntil)
          # ONly private banks
   if (samSpec$excludeStateOwn == TRUE){
     sam <- filter(sam, GRUPO_ID_UNI != 4 & GRUPO_ID_UNI !=5)
   }
  ...
  # Save sample
  saveRDS(sam, file=str_c(samSpec$outPath, 'sam_', samSpec$name, '.rds'))
  
  X <- sam %>% select(all_of(varsList))
  # Select entities at the beginiing of the sample
  failureDate <- sam %>% filter(FECHAdata==1997.4) %>% select(IDENT, EXIT_Q)
  NROW(unique(sam$IDENT))
  anti_join(sam, failureDate, by='IDENT')
  # 151 (1997q4) is the # of quarters since Stata date 0
  failureDate <- -151
  sum(failureDate<10)
    # Export variables
  write.table(failureDate, file=str_c(samSpec$outPath, 'failureDate_', samSpec['name'], '.csv'), 
            sep=', ',
            row.names = F, col.names = F)
  write.table(cbind(1, X), 
            file=str_c(samSpec$outPath, 'X_', samSpec['name'], '.csv'), 
            sep=', ',
            row.names = F, col.names = F)
  ...
  desc <- tibble('N' = length(unique(sam$IDENT)),
              ...
              'Failure until' = samSpec$failureUntil,
  ...
  saveRDS(p, file=str_c( 'dur_quarterly_privBanks_2vars_p_', samSpec$name, '.rds'))
  
  list('desc' = desc, 'descVars' = descVars, 'corr'=corr, 'p' = p)
})

names(r1) <- samsSpecs$name
...
colnames(t) <- c('Feature', samsSpecs$name )
...
tMeans <- map_dfr(r1, function(r) {
      ...
      t(.) %>% 
      as_tibble(., rownames='row_names')
colnames(tMeans) <- c('Variable', samsSpecs$name )
...
tSDs <- map_dfr(r1, function(r) {
      ...
      t(.) %>% 
      as_tibble(., rownames='row_names')
colnames(tSDs) <- c('Variable', samsSpecs$name )
