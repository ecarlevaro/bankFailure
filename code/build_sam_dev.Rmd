...
author: "Emi"
params:
  IDSAM: "A99_pastAvgW_creWs_b97q4_s03q4"
output: 
  html_document:
    code_folding: hide
...
if (SS$W.WVAR == '') {
  ...
  #
  samBanks = banks
  samRelations = avgRelations # no need to exclude banks not arleady excluded above
  excBanks <- tibble()
...
} else 
{
  ...
  if (SS$W.WVAR == "W_A_PR") {
    ...
    excBanks <- anti_join(banks, 
            tibble('IDENT' = avgRelations$IDENT_ACREEDORA))
     # if STRICT, I also exclude banks that dont lend
    if (SS$W.STRICT == TRUE) {
      excBanks <- rbind(excBanks, 
                        anti_join(banks, tibble('IDENT' = avgRelations$IDENT_DEUDORA)))
    }
    # NEW bank sample. Exclude banks that don't lend
    ...
    samRelations <- avgRelations %>%
      ...
      # Thus IDENT_ACREEDORA and IDENT_DEUDORA have to have a match in samBank
      semi_join(., select(samBanks, IDENT),  by= c('IDENT_ACREEDORA' = 'IDENT')) %>%
      semi_join(., select(samBanks, IDENT),  by= c('IDENT_DEUDORA' = 'IDENT')) %>%
      # The first column is 'from' the second 'to' in a directed graph
      select(., c(IDENT_ACREEDORA, IDENT_DEUDORA, WEIGHT))
  }  
  else   
  {
    # DEBTOR PERSPECTIVE (banks that don't borrow are excluded)
    excBanks <- anti_join(banks, 
                          tibble('IDENT' = avgRelations$IDENT_DEUDORA))
    # if STRICT, I also exclude banks that dont lend
    if (SS$W.STRICT == TRUE) {
      excBanks <- rbind(excBanks, 
                        anti_join(banks, tibble('IDENT' = avgRelations$IDENT_ACREEDORA)))
    }
    # NEW bank sample. Exclude banks that don't borrow
    samBanks <- filter(banks, !(IDENT %in% excBanks$IDENT))
    samRelations <- avgRelations %>%
      # W can only contains entities that are in the NEW sample samBanks
      # Thus IDENT_ACREEDORA and IDENT_DEUDORA have to have a match in samBank
      ...
      semi_join(., select(samBanks, IDENT),  by= c('IDENT_DEUDORA' = 'IDENT')) %>%
      # The first column is 'from' the second 'to' in a directed graph
      select(., c(IDENT_DEUDORA, IDENT_ACREEDORA, WEIGHT))
  }
}
...
These banks are excluded because they don't borrow or lend.
```{r}
select(excBanks, NOMRED, IDENT, exitDateQ) %>%
  datatable(.)
```
...
I exclude `r NROW(excBanks)` because they are not debtors. Now I have an directed graph.
...
if (file.exists(fileName)) { 
  ...
  saveRDS(list('y'=y, 'X'=X, 'network'=network, 
