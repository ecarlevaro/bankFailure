...
specs <- list('idSample' = 'A99_Wtill98q4_creW_b97q4_s03q4',
      'savingFolder' = 'output/linearOLS/Annual99/',
...
## Run!
```{r estimation_sar}
model <- lm(formula ='y ~ ActivoN + C8Est_w + CAR_IRR_3A6+ P_ROA+ P_DEP_ARS_RATE +
...
pValFtest <- 1 - pf(regSumm$fstatistic['value'], 
                    df1=regSumm$fstatistic['numdf'], df2=regSumm$fstatistic['dendf'])
#outList <- list(NULL)
...
estimatesTibble <- as_tibble(regSumm$coefficients) %>%
  ...
  mutate('MODEL_RSQ_ADJ' = regSumm$adj.r.squared,
         'MODEL_N' = NROW(regSumm$residuals),
         'MODEL_F_TEST_PVALUE' = pValFtest)

write_excel_csv(estimatesTibble, file = paste0('C:/Users/emi.ABLE-22868/OneDrive/UWA PhD/bankFailure/output/linearOLS/results_linearOLS.csv'),
                append=TRUE)
```